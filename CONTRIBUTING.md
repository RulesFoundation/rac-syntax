...
Thanks for helping improve RuleSpec syntax tooling.
...
cd packages/prism-rulespec && bun run test:watch   # single package, watch mode
```

`prism-rulespec` and `prism-catala` use Vitest with V8 coverage. `vscode-rulespec` uses Bun's built-in test runner against fixture files.

## Coverage policy

The Vitest configs for `prism-rulespec` and `prism-catala` set coverage thresholds at 80% (statements, branches, functions, lines) rather than 100%. Rationale:
...
The RuleSpec language is defined in two places: `packages/prism-rulespec/src/index.ts` (canonical keyword list) and `packages/vscode-rulespec/syntaxes/rulespec.tmLanguage.json` (TextMate grammar used by VS Code). When you change one, change the other and add a test in each package.

A comment at the top of `packages/prism-rulespec/src/index.ts` points at the files that need to stay aligned.
